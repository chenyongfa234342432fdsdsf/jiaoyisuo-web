...
} from '../../common/use-api-all-market-trade-pair/index'
import useApiAllCoinSymbolInfo from '../../common/use-api-all-coin-symbol-info'

export function useWsMarketSpotFavouriteDefault({ apiParams }: { apiParams?: YapiGetV1FavouriteDefaultApiRequest }) {
  const [apiData, setApiData] = useSafeState<WsFavouriteDefaultDataType[]>([])
...
export function useWsMarketFuturesFavouriteDefault({ apiParams }: { apiParams?: YapiGetV1FavouriteDefaultApiRequest }) {
  const [apiData, setApiData] = useSafeState<WsFavouriteDefaultDataType[]>([])
  const resolvedData = useWsFuturesMarketTradePairSlow({ apiData })
