...
import { getMemberGeeTestInitConfig, postMonkeyMemberVerifyGeeTest } from '@/apis/user'
...
function InitGeeTest(
  ...
  window.initGeetest(
    ...
    captcha => {
      ...
      captcha
        .onSuccess(async () => {
          const res = await postMonkeyMemberVerifyGeeTest({ account, gtChallenge: geetest.challenge })
...
function Geetest({ isShow, account, operateType, onSuccess, onError }: GeetestProps) {
  ...
  const geeTestInit = async () => {
    const res = await getMemberGeeTestInitConfig({ account, clientType: PlatformTypeEnum.web, operateType })
    if (res.isOk) {
