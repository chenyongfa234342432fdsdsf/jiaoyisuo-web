...
function callHookWithTimeout(call, hookName, hookFilePath) {
    ...
    const t1 = setTimeout(() => {
        const msg = `${assert_1.errorPrefix}[Warning] The ${hookName}() hook of ${hookFilePath} is taking more than ${(0, humanizeTime_1.humanizeTime)(timeoutWarn)}`;
        console.warn(msg);
    }, timeoutWarn);
    const t2 = setTimeout(() => {
        const err = (0, assert_1.getProjectError)(`Hook timeout: the ${hookName}() hook of ${hookFilePath} didn't finish after ${(0, humanizeTime_1.humanizeTime)(timeoutErr)}`);
        reject(err);
    }, timeoutErr);
    (async () => {
        try {
