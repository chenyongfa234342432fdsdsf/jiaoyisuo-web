...
function buildConfig() {
    return {
        ...
        async configResolved(config) {
            ...
            (0, helpers_1.modifyResolvedConfig)(config, { build: { rollupOptions: { input } } });
        },
        config(config) {
            return {
                build: {
...
async function entryPoints(config) {
    const ssr = (0, utils_1.viteIsSSR)(config);
    const pageFiles = await (0, helpers_1.findPageFiles)(config, ssr ? ['.page', '.page.server'] : ['.page', '.page.client']);
    const pageFilesObject = {};
    pageFiles.forEach((p) => (pageFilesObject[removeFileExtention(p.slice(1))] = (0, utils_1.makeFilePathAbsolute)(p, config)));
    if (ssr) {
        return {
