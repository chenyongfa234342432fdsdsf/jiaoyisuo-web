...
function vaviteReloaderPlugin({
  ...
  async function loadEntry() {
    ...
    const resolved = await viteServer.pluginContainer.resolveId(entry, void 0, {
      ssr: true
    });
    if (!resolved) {
    ...
    for (const id of entryDeps) {
      ...
      for (const newDep of module2.ssrTransformResult.deps || []) {
        ...
        if (newDep.startsWith("/@id/")) {
          ...
          const resolved2 = await viteServer.pluginContainer.resolveId(newDep, id, {
            ssr: true
          });
          if (!resolved2) {
            continue;
    ...
    load(id, options) {
      if (id === "virtual:vavite-http-dev-server") {
        if (!(options == null ? void 0 : options.ssr)) {
          this.error("'vavite/http-dev-server' module is only available in SSR mode");
        }
